...
const Labels: React.FC<LabelListProps<Item>> = ({items,label, setSelectedChips, selectedChips}) => {
            ...
            <View style={commonItemStyles.tagChipsContainer}>
                {items.map((item) => (
                    <Chip
                        ...
                        selected={selectedChips && selectedChips.some((selectedItem) => selectedItem.id === item.id)}
                    >
                        {item.name}
                    </Chip>
                ))}
